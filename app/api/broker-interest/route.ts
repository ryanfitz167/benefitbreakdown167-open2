...
import { sanitize, validateBroker } from "../lead/validate";
...
export async function POST(req: Request) {
  try {
    ...
    const payload: BrokerLead = sanitize({
      type: "broker",
      email: raw?.email,
    ...
    const errs = validateBroker(payload);
    if (errs.length) return bad(errs.join(", "), 400);

    const upstream = await postToGAS(payload, "broker");
    if (!upstream?.ok) return bad(upstream?.error || "upstream_error", 502);

    await notifySlack(payload);
    return NextResponse.json({ ok: true });
  } catch (err: any) {
    console.error("[broker] proxy_error", err);
    return bad("proxy_error", 502);
  }
}

export async function GET() {
  return NextResponse.json({ ok: true, message: "broker-interest alive" });
}
